			...
			*/
			...
			public:
					...
					, m_column(column)
				{
					...
					m_rowArr.reserve(m_row);
					memset(m_elemArr, 0, sizeof(ElementType) * arrSize);

					for (uint64 i = 0 ; i < m_row; ++i)
					{
						m_rowArr.emplace_back(new RowArray(m_elemArr + row * m_column, m_column));
					}
				}
		...
		//---------------------------------------
			...
			*/
			...
			public:
				...
				/// @param row 行のインデックス
				/// @return 配列[行のインデックス]の参照
				RowArray operator[](uint64 row) const&&
				{
						return at_impl(row);
				}
			// Private
